...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // A. Add to Cart Logic (AJAX SUBMISSION)
        if (addToCartForm) {
            addToCartForm.addEventListener('submit', function(e) {
                ...
                .then(response => {
                    ...
                    if (!response.ok) {
                         // Ini menangkap error dari server (misal validasi 400)
                         return response.json().catch(() => {
                            // Jika response BUKAN JSON (misal HTML Error Page), kita throw error di sini
                            throw new Error("Server mengembalikan error non-JSON. Cek routes dan controller Anda.");
                         }).then(err => {
                            throw new Error(err.message || 'Error saat memproses keranjang'); 
                         });
                    }
                ...
                .then(response => {
                    ...
                    if (!response.ok) {
                         return response.json().then(err => { 
                            throw new Error(err.message || 'Gagal memperbarui Wishlist.'); 
                         });
                    }
                    return response.json();
                })
