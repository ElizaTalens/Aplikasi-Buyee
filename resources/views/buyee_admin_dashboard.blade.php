...
async function loadOrders() {
    try {
        ...
        if (orders.length === 0) {
             // ... (Fallback jika tidak ada pesanan) ...
             tbody.innerHTML = `<tr><td colspan="7" class="text-center text-muted py-4">Tidak ada pesanan ditemukan.</td></tr>`;
            dashboardTbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted py-4">Tidak ada pesanan terbaru.</td></tr>`;
            return;
        }
...
// ** FUNGSI UNTUK MENGHUBUNGKAN PENCARIAN & FILTER **
function filterOrders() {
    // Mendapatkan nilai dari input search
    const queryInput = document.querySelector('#orders-section input[type="search"]');
    const statusSelect = document.querySelector('#orders-section .form-select');
    
    const query = queryInput ? queryInput.value : '';
    const status = statusSelect ? statusSelect.value : '';

    // Panggil loadOrders dengan filter baru
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (orderSearchInput) {
        orderSearchInput.addEventListener('input', filterOrders); // Trigger saat ketik
    }
    if (orderStatusSelect) {
        orderStatusSelect.addEventListener('change', filterOrders); // Trigger saat status diubah
    }
